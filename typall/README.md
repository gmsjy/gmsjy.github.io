...
`--to zhihu` 生成知乎编辑器可粘贴的富文本 HTML 片段（同样纯本地导出）：

- **公式转 LaTeX**：typst 数学源码经内置转换器翻译为 LaTeX（分数/根号/上下标/希腊字母/
  极限求和积分等高频结构），块级公式输出 `$$ … $$` 独立段、行内公式输出
  `<span class="ztext-math">$ … $</span>`——知乎编辑器对 LaTeX 公式有原生支持；
- **正文最小样式**：知乎对富文本的保留比公众号宽容，不做全套内联样式，只保证
  结构（标题/段落/列表）与图片链接；SVG 插图原样内嵌；
- **配对保护**：公式渲染物与源码数量不匹配（动态生成公式等罕见场景）时
  保守保留原始渲染物，不做部分替换；
- 产物输出到 `publish/zhihu/<slug>.html`，内容指纹记账（未变自动跳过）。
