...
// ---------- 六段结构小节标题 ----------
#let section(tag, title) = heading(level: 2, outlined: true)[
...
// ---------- 插图（编号 + 图注） ----------
// ⚠ typst 0.15 HTML 导出会丢弃 align() 的全部子内容（含文本）——
// 一切居中一律改走 html.elem div；分页目标（VS Code 预览 / PDF）仍走 align。
#let centered(body) = context {
  if target() == "html" {
    let h = eval("html")
    h.elem("div", attrs: (style: "text-align:center"), body)
  } else {
    align(center, body)
  }
}

// 画布版：html 下先把 canvas 渲染为 frame 再套居中 div
#let fig-center(body) = context {
  if target() == "html" {
    let h = eval("html")
    h.elem("div", attrs: (style: "text-align:center"), h.frame(body))
  } else {
    align(center, body)
  }
}
...
#let figure-block(num, caption, body) = block(above: 1.1em, below: 1.2em, breakable: false)[
  #fig-center(body)
  #v(0.35em)
  #centered(text(size: 8.5pt, fill: gray-line, weight: 500)[图 #num　#caption])
]
